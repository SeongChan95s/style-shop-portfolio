...
import { magazineEditFormSchema } from '@/app/lib/zod/schemas/admin';
...
export async function PUT(req: NextRequest) {
	try {
		...
		const validation = magazineEditFormSchema.safeParse(parsedFormData);
		...
		const { _id, name, title, body, url, productGroupsId, keywords, images } = validationData;
		...
		// productGroupsId를 ObjectId 배열로 변환
		const productGroupsIdObjectIds = productGroupsId.map(id => new ObjectId(id));

		// MongoDB 업데이트 또는 생성
		if (isNew) {
			await db.collection<Magazine>('contents').insertOne({
				...
				url: url || '',
				images: finalImages,
				productGroupsId: productGroupsIdObjectIds.map(id => id.toString()),
				keywords,
				keyword: keywords, // Magazine 타입의 keyword 필드 (타입 호환성)
				products: [] // Magazine 타입의 products 필드
			});
		} else {
			await db.collection<Magazine>('contents').updateOne(
				{ _id: new ObjectId(_id) },
				{
					$set: {
						name,
		...
		return NextResponse.json(
			{
				success: true,
				message: isNew ? '매거진을 생성했습니다.' : '매거진 정보를 수정했습니다.'
			},
			{ status: 200 }
		);
	} catch (error) {

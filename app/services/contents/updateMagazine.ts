import { MagazineFormData } from '@/app/(route)/(desktop)/admin/content/magazine/edit/[id]/MagazineForm';
import { HTTPError } from '../HTTPError';
import { omit } from 'lodash';
import { useSystemAlertStore } from '@/app/store';

export const updateMagazine = async (data: MagazineFormData, isNew: boolean) => {
	...
	const response = await fetch(
		`${process.env.NEXT_PUBLIC_CLIENT_URL}/api/content/updateMagazine`,
		{
			method: 'PUT',
			body: formData
		}
